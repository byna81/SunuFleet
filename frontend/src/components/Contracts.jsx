...
const Contracts = ({ contracts, setContracts, drivers, vehicles, currentUser, hasPermission }) => {
  ...
  const handleAddContract = async (e) => {
    ...
    try {
      ...
      const vehicle = vehicles.find(v => String(v.id) === String(newContract.vehicleId));

      const status = hasPermission('all') ? 'active' : 'pending';

      // On garde les champs "métier" + quelques meta (avec fallback auto si colonnes absentes)
      const payload = {
        ...
        // champs dérivés (si présents en DB)
  ...
  const handleEditContract = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        // Si pas admin: repasse en pending (si colonne existe)
  ...
  const handleValidateContract = async (contractId) => {
    ...
    setContracts((contracts || []).map(c => (c.id === data.id ? { ...c, ...data } : c)));
