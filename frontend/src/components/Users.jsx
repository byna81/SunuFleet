// Users.jsx - Gestion des utilisateurs (Admin uniquement) - AVEC SUPABASE
// ✅ INSERT/DELETE en base
// ✅ Vérifie unicité email + username avant création
  ...
  const handleAddUser = async (e) => {
    ...
    try {
      ...
      if (emailCheckError) {
        ...
        alert('❌ Erreur vérification email: ' + emailCheckError.message);
        return;
      }
      ...
      if (usernameCheckError) {
        ...
        alert("❌ Erreur vérification nom d'utilisateur: " + usernameCheckError.message);
        return;
      }
      ...
      if (error) {
        ...
        // message propre si contrainte unique quand même (cas de course)
        if (String(error.message || '').toLowerCase().includes('users_email_key')) {
          alert("❌ Cet email est déjà utilisé. Mets un autre email.");
          return;
        }
        if (String(error.message || '').toLowerCase().includes('users_username_key')) {
          alert("❌ Ce nom d'utilisateur est déjà utilisé. Choisis-en un autre.");
          return;
        }
